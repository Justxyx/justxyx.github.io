...
GCC 决定C 版本， GCC 决定代码怎么被编译（语法、标准、优化），glibc 提供运行时库函数（如 malloc/printf），而真正让程序执行的是操作系统内核；同时二进制通常只能向后兼容 glibc（新系统跑老程序），Makefile 里指定 C 标准是为了保证代码语义一致、避免环境差异和 UB 问题。
