...
- `str[x]`,[ ]输入参数为`string::size_type`类型，给出`int`整型也会自动转化为该类型
    ...
    ~~~c
    void test02(){
    string str = "Hey jay , long time no see";

    for (const auto &item : str){
        cout << isalnum(item) << endl;
        isalpha(item);// 字母
        iscntrl(item);// 控制字符
...
- **difference_type**：保证足够大以存储任何两个迭代器对象间的距离，可正可负。
    ...
    vector<int>::difference_type  s = end - begin;
    cout << s << endl;
    }
    ~~~
...
- 初始化：`char input_buffer[buffer_size];`，长度必须是const表达式，或者不写，让编译器自己推断。维度必须是一个**常量表达式,constexpr**. 必须是const 这点不必吧。。。
- 数组不允许直接赋值给另一个数组。
